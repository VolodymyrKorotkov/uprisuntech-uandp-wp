    ...
    if (is_llms_account_page()) {
        ...
        if (!is_user_logged_in()) {
            ...
            if ($endpointSlug == 'dashboard') {
                ...
                wp_enqueue_script('lifterlms-llms', get_template_directory_uri() . '/assets/js/llms.js', array('jquery'), _S_VERSION, true);
                wp_enqueue_script('lifterlms-profile', get_template_directory_uri() . '/assets/js/profile-settings.js', array('jquery'), 213454343, true);
...
function get_municipality_requests($args = []) {
    ...
    if (!empty($args['parseRequests']) && $args['parseRequests']) {
        foreach ($requests as $key => $request) {
            $requests[$key]->received = date( 'Y.m.d', strtotime( $request->received ) );
            $requests[$key]->due_to = !empty($request->due_to) && !preg_match('/0000/', $request->due_to)? date( 'Y.m.d', strtotime( $request->due_to ) ) : '';
...
function get_new_jwt_token($code, $state, $uuid=0) {
    // Предполагается, что у вас есть функция для получения user_id по uuid
    $user_id = get_user_id_from_uuid($uuid);

    if (isset($user_id)) {
        // Получение refresh_token из wp_usermeta
        $refresh_token = get_user_meta($user_id, 'refresh_token', true);

        // Запрос на обновление токена
        $response = wp_remote_get("https://digitalocean.uprisun.dev/api/v1/auth/refresh-token", array(
            'headers' => array(
            ...
            $body = wp_remote_retrieve_body($response);
            $data = json_decode($body, true);
            return $data; // Возвращает новый JWT токен

    } else {
        $token_response = wp_remote_get("https://digitalocean.uprisun.dev/api/v1/auth/token?code={$code}&state={$state}");
        if (is_wp_error($token_response)) {
            echo 'Ошибка при получении токена: ' . $token_response->get_error_message();
            exit;
        }
        ...
        // Сохранение refresh_token в wp_usermeta
...
function get_user_data($jwt_token) {
    $user_response = wp_remote_get("https://digitalocean.uprisun.dev/api/v1/users/profile", array(
        'headers' => array(
...
//    if($_COOKIE['901a6b96dbc8033f4d411c6e0fb53113']!=$state) return;
    ...
    if ($isDia && $code) {
        // Запрос на получение JWT токена и refresh token
        if(isset($uuid)){
        ...
        if (is_wp_error($user_response)) {
            echo 'Ошибка при получении информации о пользователе: ' . $user_response->get_error_message();
            exit;
        }
    ...
    if(isset($user_data['date_of_birth'])){
        ...
        if(isset($user_data['refresh_token'])){
            update_user_meta($user_id, 'refresh_token', $refresh_token);
        }
        ...
        if (isset($user_data['middleName'])) {
            update_user_meta($user_id, 'middle_name', $user_data['middleName']);
        }

        if (isset($user_data['edrpouCode'])) {
            update_user_meta($user_id, 'edrpou_code', $user_data['edrpouCode']);
        }
        ...
        if (!is_wp_error($user_id)) {
            ...
            wp_set_auth_cookie($user_id);
            wp_authenticate($user_data['email'],$user->user_pass);
...
function authFakeDia()
{
        ...
        $get_user = create_or_authenticate_wp_user($user_data);
        if (isset($user_data['uuid'])) {

        $user_id=$get_user['user_id'];

            update_user_meta($user_id, 'uuid', $user_data['uuid']);
            update_user_meta($user_id, 'refresh_token', $get_user['refresh_token']);
        }
...
//        'edrpouCode' => $faker->randomNumber(8),
...
function create_or_authenticate_wp_user($data) {
    ...
    if (!$user) {
        ...
        if (isset($data['edrpouCode'])) {
            update_user_meta($user_id, 'edrpou_code', $data['edrpouCode']);
        }
...
function municipality_add_request(array $params=[]) {
    ...
    $sql = "SELECT * FROM {$table_name} WHERE `user_id`={$user_id} AND `status`='Await processing'";
...
// Функция обработки запроса на изменение пароля
function update_user_password($request) {
    ...
    update_wp_user_password($password, $user_id);

    // Очистка всех сессий пользователя и установка новой сессии
    wp_clear_auth_cookie();
    wp_set_current_user($user_id);
    wp_set_auth_cookie($user_id);

    // Дополнительно можно войти пользователя снова
...
//возможность смотреть чужие сертификаты
