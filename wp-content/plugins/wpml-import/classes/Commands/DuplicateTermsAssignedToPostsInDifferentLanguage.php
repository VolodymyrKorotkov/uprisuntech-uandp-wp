	...
	public function run( Collection $args = null ) {
		...
		foreach ( $items as $item ) {
			...
			if ( $termToDuplicate ) {
				...
				$duplicatedTerm = Language::switchAndRun( $item->target_language_code, function() use ( $termToDuplicate, $termArgs ) {
					return wp_insert_term( $termToDuplicate->name, $termToDuplicate->taxonomy, $termArgs );
				} );

				if ( isset( $duplicatedTerm['term_taxonomy_id'] ) ) {
					$this->sitepress->set_element_language_details(
						$duplicatedTerm['term_taxonomy_id'],
