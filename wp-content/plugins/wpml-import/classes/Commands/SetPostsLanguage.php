	...
	protected function getPendingItems( $limit = null ) {
		return $this->getResultsWithLimit(
			...
			SELECT
				...
				(SELECT meta_value FROM {$this->wpdb->postmeta} WHERE post_id = pm.post_id AND meta_key='" . Fields::LANGUAGE_CODE . "') AS language_code,
				(SELECT meta_value FROM {$this->wpdb->postmeta} WHERE post_id = pm.post_id AND meta_key='" . Fields::SOURCE_LANGUAGE_CODE . "') AS source_language_code
			FROM {$this->wpdb->postmeta} pm
			WHERE meta_key = '" . Fields::TRANSLATION_GROUP . "'
				HAVING language_code IS NOT NULL
			ORDER BY meta_value ASC,
				CASE WHEN (SELECT meta_value FROM {$this->wpdb->postmeta} WHERE post_id = pm.post_id AND meta_key='" . Fields::SOURCE_LANGUAGE_CODE . "') = ''
				THEN 0 ELSE 1 END
			",
			$limit
		);
