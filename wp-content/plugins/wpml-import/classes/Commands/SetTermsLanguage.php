...
class SetTermsLanguage extends Base\SetEntitiesLanguage {
	...
	protected function getPendingItems( $limit = null ) {
		return $this->getResultsWithLimit(
			...
			SELECT
				...
				tm.term_id,
				tt.taxonomy,
				(SELECT meta_value FROM {$this->wpdb->termmeta} WHERE term_id = tm.term_id AND meta_key='" . Fields::LANGUAGE_CODE . "') AS language_code,
				(SELECT meta_value FROM {$this->wpdb->termmeta} WHERE term_id = tm.term_id AND meta_key='" . Fields::SOURCE_LANGUAGE_CODE . "') AS source_language_code
			...
			ORDER BY meta_value ASC,
				CASE WHEN (SELECT meta_value FROM {$this->wpdb->termmeta} WHERE term_id = tm.term_id AND meta_key='" . Fields::SOURCE_LANGUAGE_CODE . "') = ''
				THEN 0 ELSE 1 END
			",
			$limit
		);
	...
	protected function getTridAndType( $source, $translations ) {
		...
		if ( $source ) {
			...
			if ( ! $trid ) {
				$this->sitepress->set_element_language_details( $source->element_id, $elementType, $trid, $source->language_code );
				$trid = $this->sitepress->get_element_trid( $source->element_id, $elementType );
			}
		} else {
