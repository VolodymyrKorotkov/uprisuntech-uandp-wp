...
class IS_Search_Form
{
	...
	public static function get_template($args = '')
	{
		...
		$search_form = apply_filters(
			'is_search_form_default_pack',
			$search_form,
			$args
		);
	...
	public function form_html($args = '', $display_id = '')
	{
		...
		if (!$enabled_customization && $args['id']) {
			...
			if (isset($_ajax['enable_ajax'])) {
				...
				if (!ivory_search_is_json_request() && isset($_settings['highlight_terms'])) {
					...
					if (isset($_settings['highlight_color']) && !empty($_settings['highlight_color']) && !is_search()) { ?>
						<style type="text/css" media="screen">
							#is-ajax-search-result-<?php esc_html_e($args['id']); ?> .is-highlight {
								background-color:
									<?php esc_html_e($_settings['highlight_color']); ?>
									!important;
							}

							#is-ajax-search-result-<?php esc_html_e($args['id']); ?> .meta .is-highlight {
				...
				$result_box_max_height = isset($_ajax['result_box_max_height']) ? $_ajax['result_box_max_height'] : '680';
			...
			if (isset($_ajax['enable_ajax'])) {
				...
				$close_image = isset($settings['loader-image']) ? $settings['loader-image'] : IS_PLUGIN_URI . 'public/images/icon-close.svg';
				if ($loader_image) {
					$result .= '<span class="is-loader-image" style="display: none;background-image:url(' . esc_url($loader_image) . ');" ></span>';
					$result .= '<span class="clear-search-request" style="display: none; background-image:url(' . esc_url($close_image) . ');" ></span>';
				}
			}
			$result .= '</label>';
