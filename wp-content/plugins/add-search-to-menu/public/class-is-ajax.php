...
class IS_Ajax
{
			...
			<div class="<?php esc_attr_e($wrapper_class); ?>">
				<?php foreach ($tags as $key => $tag) { ?>
					<div data-id="<?php echo esc_attr($tag['term_id']); ?>" class="is-ajax-search-post">
						<span class="is-ajax-term-label">
							<?php echo esc_html($term_title); ?>
						</span>
						<div class="is-title">
							<a href="<?php echo esc_url($tag['url']); ?>" data-id="<?php echo esc_attr($tag['term_id']); ?>"
								data-slug="<?php echo esc_attr($tag['slug']); ?>">
								<?php echo esc_attr($tag['name']); ?> (<span class="is-term-count">
									<?php echo esc_attr($tag['count']); ?>
								</span>)
							</a>
						</div>
					</div>
	...
	function get_product_by_tax_id($field, $cat_id, $taxonomy)
	{
		...
		if ($products->have_posts()) {
			...
			while ($products->have_posts()) {
				...
				$product_count++;
				?>
				<div data-id="<?php echo esc_attr($cat_id); ?>"
					class="is-ajax-search-post-details is-ajax-search-post-details-<?php echo esc_attr($cat_id); ?>">
	...
	{
		$image = '';
		$image_size = apply_filters('is_ajax_image_size', 'medium');
		$temp_id = 0;
		...
		if (!$is_markup && isset($field['show_image']) && $field['show_image']) { ?>
			<div class="left-section">
				<div class="thumbnail">
					<a href="<?php echo get_the_permalink($temp_id); ?>">
			...
			<div class="is-title">
				<a href="<?php echo get_the_permalink($post->ID); ?>">
					<?php if ($product && isset($field['show_featured_icon']) && $field['show_featured_icon'] && $product->is_featured()) { ?>
						<svg class="is-featured-icon" focusable="false" aria-label="<?php _e("Featured Icon", "add-search-to-menu"); ?>"
							version="1.1" viewBox="0 0 20 21" xmlns="http://www.w3.org/2000/svg"
							xmlns:sketch="http://www.bohemiancoding.com/sketch/ns" xmlns:xlink="http://www.w3.org/1999/xlink">
							<g fill-rule="evenodd" stroke="none" stroke-width="1">
								<g transform="translate(-296.000000, -422.000000)">
		...
		if (!$is_markup && isset($field['show_tags']) && $field['show_tags']) { ?>
			...
			if ($terms && !is_wp_error($terms)) { ?>
				...
				<div class="category-tags">
					<ul class="category-tags__tags-list">
						<?php foreach ($terms as $key => $term) { ?>
							<li>
								<a href="<?php echo get_term_link($term->term_id, $post->post_type . '_tag'); ?>"
									class="btn__accessory btn_bg_no-selected">
				...
				<span class="is-meta-category">
					...
					<span class="is-cat-links">
						<?php foreach ($terms as $key => $term) {
							if ($key) {
								echo ', ';
							} ?><a href="<?php echo get_term_link($term->term_id, $tax_name); ?> " rel="tag">
								<?php esc_html_e($term->name); ?>
							</a>
						<?php } ?>
					</span>
