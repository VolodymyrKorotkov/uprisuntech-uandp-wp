...
if ($posts->posts) {
	...
	foreach ($posts->posts as $post):
		...
		?>
				<div data-id="<?php echo esc_attr($post->ID); ?>"
					class="is-ajax-search-post is-ajax-search-post-<?php echo esc_attr($post->ID) . ' ' . esc_attr($product_class); ?>">
					<!-- Header -->
...
if (isset($field['show_more_result']) && $field['show_more_result'] && ($posts->found_posts > ($posts_per_page * $page))) {
	...
	<div class="is-show-more-results <?php esc_attr_e($show_more_class); ?>" data-page="<?php echo $next_page; ?>">
		<?php /* <div class="is-show-more-results-text"><?php echo wp_kses_post( $field['more_result_text'] .' <span>('. ( $posts->found_posts - ( $posts_per_page * $page ) ) .')</span>'); ?></div> */ ?>
		<div class="is-show-more-results-text">
			<?php echo wp_kses_post($field['more_result_text']); ?>
			...
			if ($posts->posts) { ?>
				<div class="is-ajax-search-posts-details">
					...
					foreach ($posts->posts as $post):
						...
						if ($product) { ?>
							<div data-id="<?php echo esc_attr($post->ID); ?>"
								class="is-ajax-search-post-details is-ajax-search-post-details-<?php echo esc_attr($post->ID) . ' ' . esc_attr($product_class); ?>">
								<div class="is-search-sections">
									<?php $this->image_markup($field, $post); ?>
									<div class="right-section">
										<?php
