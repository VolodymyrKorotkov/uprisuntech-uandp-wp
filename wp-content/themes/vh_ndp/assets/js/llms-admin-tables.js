	...
	var AdminTables = function() {
		...
		this.bind = function() {
			...
			this.$tables.each( function() {
				...
				$table.on( 'change', '#quizzesByCourseSelect', function( e ) {
					e.preventDefault();
					if (this.value) {
						self.set_arg($table, 'data-course_id', this.value);
						$table.removeAttr('data-search')
					} else {
						$table.removeAttr('data-course_id')
					}
				} );
		...
		this.export = function( $table, $btn, filename ) {
			...
			if (data['handler'] === 'Quizzes' && this.get_arg($table, 'data-course_id')) {
				data['course_id'] = this.get_arg($table, 'data-course_id')
			} else if (data['search'] === '' && data['handler'] === 'Quizzes' && this.get_arg($table, 'data-search')) {
				data['search'] = this.get_arg($table, 'data-search')
			}
			if ($btn.is('.llms-button-export-all-quizzes')) {
				data['all_quizzes_data'] = true;
			}
		...
		this.get_arg = function( $table, item ) {

			if ( item ) {
				return ( $table.attr( item ) ) ? $table.attr( item ) : false;
			}

		};
		this.set_arg = function( $table, item, value ) {
		...
		this.search = function( $table, $input ) {
			...
			if (len > 3 && $table.is('.llms-gb-table-quizzes')) {
				this.set_arg($table, 'data-search', val);
				$table.removeAttr('data-course_id');
			}
