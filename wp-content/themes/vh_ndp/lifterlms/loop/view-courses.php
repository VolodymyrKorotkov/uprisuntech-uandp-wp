...
?>
<div class="my-courses__item" data-id="<?php echo $course->get('id'); ?>">
    <div class="my-courses__item-header">
        <div class="my-courses__item-header__left">
            <div class="my-courses__item-about">
                ...
                <?php if ($course->get_difficulty()): ?>
                    <span class="my-courses__item-complexity" title="<?php _e('Difficulty', 'ndp'); ?>"><?php echo $course->get_difficulty(); ?></span>
                <?php endif; ?>
                <?php if (!empty($tags)): ?>
                    <?php
                    $titleTags = array_slice($tags, 1);
                    $titleTags = implode(',', array_column($titleTags, 'name'));
                    ?>
                    <?php if (count($tags) > 1): ?>
                        <span class="my-courses__item-more" title="<?php if ($titleTags && is_string($titleTags)) echo $titleTags; ?>">+<?php echo count($tags)-1; ?></span>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
        ...
        ?>
        <a class="my-courses__start-course" href="<?php echo get_permalink( $lesson ); ?>"><?php echo $bntText; ?></a>
    </div>
    ...
    <div class="my-courses__item-info">
        <div class="my-courses__item-provider">
            <span><?php _e('Provider', 'ndp'); ?>:</span><strong>
                <?php echo llms_get_author(
                    array(
                        'avatar' => false,
                    )
                ); ?></strong>
    ...
    <div class="my-courses__item-status">
        ...
        <div class="my-courses__item-progress">
            <span class="my-courses__item-progress-current" style="width: <?php echo $courseProgress.'%'; ?>"></span>
        </div>
        <span class="my-courses__item-percent"><?php echo $courseProgress; ?>%</span>
    </div>
    ...
    if ($certificate && is_a($certificate, 'LLMS_User_Certificate')) {
        ...
        $expiredClass = $expired? 'my-courses__item-state__invalid' : '';
    ...
    <?php if (!empty($valid)): ?>
      <span class="my-courses__item-state <?php echo $expiredClass; ?>"><?php echo $valid; ?></span>
      <span class="my-courses__item-date"><?php _e('Until', 'ndp'); ?> <?php echo $expireDate; ?></span>
    <?php endif; ?>
    </div>
    <?php endif; ?>
</div>