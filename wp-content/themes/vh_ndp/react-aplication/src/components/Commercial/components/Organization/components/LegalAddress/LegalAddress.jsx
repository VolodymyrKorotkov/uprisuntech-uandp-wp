...
function LegalAddress({data = {}, onSave = () => {}, forseShowList}) {
  const [viewList, setViewList] = useState(forseShowList ? forseShowList : Boolean(localStorage.getItem('show_LegalAddress') == 'true'));
  const [activeTab, setActiveTab] = useState('Tab1')

  const formContext = useForm({
    defaultValues: data,
    values: data,
    resolver: yupResolver(organizationsSchemas.legal_address),
    mode: 'all'
  })
  ...
  const onSubmit = (value) => {
    ...
    localStorage.setItem('show_LegalAddress', !viewList)
  ...
  const onError = (errors) => {
    if(errors && viewList) {
      setViewList(false);
      localStorage.setItem('show_LegalAddress', false)
    }
  }
        ...
        <div className={classNames(global.card, styles.LegalAddress)}>
          <div className={global.header}>
            <div className={global.row}>
              <div className={global.title}><I18n text={'Legal address'} /></div>
              {!forseShowList && <>
