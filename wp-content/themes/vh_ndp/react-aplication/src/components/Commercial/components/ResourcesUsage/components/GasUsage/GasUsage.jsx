...
import ListGasUsage from './ListGasUsage';
import {CurrencyField, NumberField} from "../../../../../NumberFields";
import {resourcesUsageSchemas} from '../../validation.schema'


function GasUsage({data = {}, onSave, forseShowList}) {
  const [viewList, setViewList] = useState(forseShowList ? forseShowList : Boolean(localStorage.getItem('show_GasUsage') == 'true'))

  const formContext = useForm({
    defaultValues: data,
    resolver: yupResolver(resourcesUsageSchemas.gas_usage),
    mode: 'all'
  });
  ...
  const onSubmit = (value) => {
    ...
    localStorage.setItem('show_GasUsage', !viewList)
  ...
  const onError = (errors) => {
    if(errors && viewList) {
      setViewList(false);
      localStorage.setItem('show_GasUsage', false)
    }
  }
      ...
      <div className={global.card}>
        <div className={global.header}>
          <div className={global.row}>
            <div className={global.title}><I18n text={'Gas usage'} /></div>
        ...
        <div className={global.body}>
          {!viewList && <>
            ...
            <div className='row'>
              ...
              {data.period_for_which_we_enter_data != 'Monthly estimate (1-12 month)' && <div className='col-md-6 mt-3 mt-md-0'>
                <NumberField
                  name={'monthly_gas_consumption'}
                  label={
                    <I18n text={data.type_tariff == 'm³' ?
            ...
            <div className='row'>
              <div className='col-md-12'>
                <TextFieldElement
                  name={'gas_supplier'}
                  label={<I18n text={'Gas supplier'}/>}
                  disabled={data.i_dont_have_this_information_right_now}
                  fullWidth
                  onChange={(e) => {
                    onSave({...data, 'gas_supplier': e.target.value})
                  }}
                />
              </div>
            </div>
            ...
            <div className='row'>
              <div className='col-md-12'>
                <UploadField
                  name={'bill_for_gas_consumption'}
                  label={<I18n text={'Bill for gas consumption'} />}
                  disabled={data.i_dont_have_this_information_right_now}
                  value={data.bill_for_gas_consumption}
                  onChange={(v) => {
                    formContext.setValue('bill_for_gas_consumption', v)
                    onSave({...data, 'bill_for_gas_consumption': v})
                  }}
                />
              </div>
            </div>
          </>}
          {viewList && <ListGasUsage data={data} /> }
        </div>
      </div>
    </FormContainer>
  )
