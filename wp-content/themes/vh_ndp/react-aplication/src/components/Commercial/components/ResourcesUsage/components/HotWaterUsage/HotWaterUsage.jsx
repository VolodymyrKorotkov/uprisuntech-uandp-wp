...
import ListHotWaterUsage from './ListHotWaterUsage';
import {CurrencyField, NumberField} from "../../../../../NumberFields";
import {resourcesUsageSchemas} from '../../validation.schema'


function HotWaterUsage({data = {}, onSave, forseShowList}) {
  const [viewList, setViewList] = useState(forseShowList ? forseShowList : Boolean(localStorage.getItem('show_HotWaterUsage') == 'true'))
  const formContext = useForm({
    defaultValues: data,
    resolver: yupResolver(resourcesUsageSchemas.hot_water_usage),
    mode: 'all'
  });

  useEffect(() => {
    if(!data?.type_tariff){
      onSave({...data, type_tariff: 'm³'});
    }
  }, [data])

  const onSubmit = (value) => {
    setViewList(!viewList);
    localStorage.setItem('show_HotWaterUsage', !viewList)
  ...
  const onError = (errors) => {
    if(errors && viewList) {
      setViewList(false);
      localStorage.setItem('show_HotWaterUsage', false)
    }
  }
      ...
      <div className={global.card}>
        <div className={global.header}>
          <div className={global.row}>
            <div className={global.title}><I18n text={'Hot water usage'} /></div>
          ...
          {!viewList && <>
            <div className='row'>
              ...
              <div className='col-md-6'>
                <RadioGroup
                  ...
                  defaultValue={data?.type_tariff || 'm³'}
                  name="type_tariff"
                ...
                >
                  <FormControlLabel value="m³" control={<Radio />} label={<I18n text="m³"/>} />
            ...
            <div className='row'>
              <div className={data.period_for_which_we_enter_data != 'Monthly estimate (1-12 month)' ?'col-md-6' : 'col-md-12'}>
                <CurrencyField
                  name={'tariff_per'}
                  label={<I18n text={`Tariff (UAH per m³)`} />}
                  fullWidth
                  onChange={(e) => {
              ...
              {data.period_for_which_we_enter_data != 'Monthly estimate (1-12 month)' &&
                <div className='col-md-6 mt-3 mt-md-0'>
                  <NumberField
                    name={'monthly_hot_water_consumption'}
                    label={<I18n text={data.type_tariff == 'm³' ?
                      data.period_for_which_we_enter_data == 'Year average' ? 'Year consumption (m³)' : 'Monthly consumption (m³)'
            ...
            <div className='row'>
              <div className='col-md-12'>
                <TextFieldElement
                  name={'hot_water_supplier'}
                  label={<I18n text={'Hot water supplier'}/>}
                  disabled={data.i_dont_have_this_information_right_now}
                  fullWidth
                  onChange={(e) => {
                    onSave({...data, 'hot_water_supplier': e.target.value})
                  }}
                />
              </div>
            </div>
            ...
            <div className='row'>
              <div className='col-md-12'>
                <UploadField
                  name={'bill_for_hot_water_consumption'}
                  label={<I18n text='Bill for hot water consumption' />}
                  disabled={data.i_dont_have_this_information_right_now}
                  value={data.bill_for_hot_water_consumption}
                  onChange={(v) => {
                    formContext.setValue('bill_for_hot_water_consumption', v)
                    onSave({...data, 'bill_for_hot_water_consumption': v})
                  }}
                />
              </div>
            </div>
          </>}
          {viewList && <ListHotWaterUsage data={data} /> }
        </div>
      </div>
    </FormContainer>
  )
